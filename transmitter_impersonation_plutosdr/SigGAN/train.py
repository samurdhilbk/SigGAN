...
class Trainer(object):
    def __init__(self, rf_system, B, T, n_authorized, g_H, d_dropout, g_lr=1e-3, d_lr=1e-3, n_sample=16, generate_samples=10000, binary_reward=True, epsilon=0.1, beta=10000, d_load_path=None):
        ...
        if d_load_path is None: self.discriminator = Discriminator()
        else: self.discriminator = Discriminator(load_path=d_load_path)
        self.discriminator.save_weights('t_disc_init_weights')
    ...
    def reset_disc(self):
        self.discriminator.load_weights('t_disc_init_weights')
    ...
    def hard_reset_generator(self, lr, epsilon, beta, binary_reward):
...
#         self.g_beta.generator.build_graph()
    ...
    def get_predicted_sequence(self):
        orig=self.rf_system.get_real_pretx_symbol_block(self.T )
        return orig, self.agent.generator.sampling_sequence_alt(self.T, orig)
    
    def get_n_received_samples_from_generator(self, n_samples=100):
        perturbed_signals=np.zeros((n_samples, 1024, 2))
        for i in range(n_samples):
            perturbed_signals[i,:]=self.rf_system.transmit_real_symbol_block(self.agent.generator.sampling_sequence_alt(self.T, self.rf_system.get_real_pretx_symbol_block(self.T )), impersonator=True)
        return perturbed_signals
    
    def test_curr_discrimintor_for(self, signals):
        n_samples = signals.shape[0]
        return self.test_discriminator(signals, np.ones((n_samples,)))
    
    def test_curr_discriminator_batch(self, n_samples=100):
        perturbed_signals=np.zeros((n_samples, 1024, 2))
        p_symbols = self.agent.generator.sampling_sequence_alt(self.T, self.rf_system.get_real_pretx_symbol_block(self.T ))
        self.rf_system.transmit_repeat_symbol_block(self.rf_system.real_to_complex(p_symbols), impersonator = True)
        for i in range(n_samples):
            perturbed_signals[i,:]=self.rf_system.get_bare_received_symbol_block(cmplx=False)
        self.rf_system.stop_transmit_repeat(impersonator = True)
...
#         print("Disc Batch Accuracy: {:.3f}".format(self.test_discriminator(perturbed_signals, np.ones((n_samples,)))))
    ...
    def predict_curr_discriminator_for_auth(self):
        signal=self.rf_system.tx_rx_real_symbol_block(self.rf_system.get_real_pretx_symbol_block(self.T ), authorized=True, tx_id=1)
        return self.predict_discriminator(np.expand_dims(signal, 0)).squeeze()
    
    def predict_curr_discriminator(self):
        perturbed_signal=self.rf_system.tx_rx_real_symbol_block(self.agent.generator.sampling_sequence_alt(self.T, self.rf_system.get_real_pretx_symbol_block(self.T )), impersonator=True)
...
#         return ((pred==np.argmax(d_out_test)).sum())/pred.shape[0]
    ...
    def save_pre_train_d(self, sess_dir, suffix=''):
        ...
        with open(sess_dir+"/discriminator_arch%s.json"%suffix, "w") as json_file:
            json_file.write(model_json)
        self.discriminator.save_weights(sess_dir+"/discriminator_weights%s.h5"%suffix)
        self.discriminator.save(sess_dir+"/discriminator_model%s.h5"%suffix)
...
#                 elif len(accus)>=10 and np.mean(accus[-3:])<0.05 and self.is_convergent(accus[-5:], sensitivity):
#                     print("\nConverged to 0%: {:.3f}, {:d}".format(np.mean(accus[-3:]), step+1))
#                     return np.mean(accus[-3:]), step+1
                elif len(accus)>=15 and self.is_convergent(accus[-5:], sensitivity):
                    print("\nConverged to {:.3f}, {:d}".format(np.mean(accus[-5:]), step+1))
                    return np.mean(accus[-5:]), step+1
...
#                                                                                                                      self.test_curr_discriminator_batch()))
            ...
            self.reflect_agent_to_beta()
            if((step+1)%1==0): print('{:d}: Disc Batch Accuracy: {:.3f}'.format(step, self.test_curr_discriminator_batch()))
                

    def get_adversarial_examples_for(self, images, labels):
        ret = np.zeros_like(images)
        for i in tqdm(range(images.shape[0])):
            ret[i,:] = self.adversarial(images[i].reshape((1, 1024, 2)), labels[i].astype('float32').reshape((1,1)), verbose=0)
        return ret
                
    def adversarial(self, image, label, verbose=0):
        # Initialize adversarial example with input image
        x = image
        epsilon = 0.01
        
        if(verbose): print(0, self.discriminator.predict(x_adv))

        # One hot encode the initial class
        target = label

        # Get the loss and gradient of the loss wrt the inputs
        loss = K.binary_crossentropy(target, self.discriminator.output)
        grads = K.gradients(loss, self.discriminator.input)

        # Get the sign of the gradient
        x_noise = K.sign(grads[0])

        # Perturb the image
        x_adv = x + epsilon*x_noise

        # Get the new image and predictions
        x_adv = sess.run(x_adv, feed_dict={self.discriminator.input:x})

        if verbose:
            preds = self.discriminator.predict(x_adv)
            print(i+1, preds)
...
#         plt.plot(image.reshape(1024,2))
#         plt.figure()
#         plt.plot(x_adv.reshape(1024,2))
#         plt.figure()
        #plt.figure()
        #plt.plot(x_adv)
        #plt.plot((image-x_adv).reshape(1024,2))
