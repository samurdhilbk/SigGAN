...
class Agent(object):
    def __init__(self, sess, B, H, lr=1e-3,beta=10000, epsilon=0.1):
        self.sess = sess
        self.B = B
        self.H = H
        self.lr = lr
        self.generator = Generator(sess, H, lr, beta, epsilon)
...
class Environment(object):
    def __init__(self, rf_system, discriminator, B, T, g_beta, n_sample=16, binary_reward=True):
    ...
    def set_reward_type(self, binary_reward):
        self.binary_reward = binary_reward
    ...
    def process_reward(self, perturbed_signal):
        trans_signal = self.rf_system.tx_rx_real_symbol_block(perturbed_signal.reshape(self.T,2), impersonator=True)
        trans_signal = trans_signal.reshape(-1,self.T,2)
        #return 1 if ((self.discriminator.predict(self.fft(trans_signal).reshape(-1,self.T)).squeeze()) > 0.5) else 0 #fft
        #return 0 if (np.argmax(self.discriminator.predict(trans_signal), axis=-1) == 5 ) else 1
        #return 1 if (self.discriminator.predict(trans_signal).squeeze() > 0.5) else 0
        if self.binary_reward: reward =  1 if (self.discriminator.predict(trans_signal).squeeze() > 0.5) else 0
        else: reward =  (self.discriminator.predict(trans_signal).squeeze() - 0.0) 
        return reward
        #return (self.discriminator.predict(trans_signal).squeeze() - 0.5)
