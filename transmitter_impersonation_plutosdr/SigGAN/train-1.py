...
#         self.g_beta.generator.build_graph()
    ...
    def test_curr_discriminator_batch(self, n_samples=100):
        perturbed_signals=np.zeros((n_samples, 1024, 2))
        p_symbols = self.agent.generator.sampling_sequence_alt(self.T, self.rf_system.get_real_pretx_symbol_block(self.T//2))
        self.transmit_repeat_symbol_block(p_symbols, tx_id, impersonator = True)
        for i in range(n_samples):
            perturbed_signals[i,:]=self.rf_system.get_bare_received_symbol_block()
        self.stop_transmit_repeat(tx_id, impersonator = True)
...
#         print("Disc Batch Accuracy: {:.3f}".format(self.test_discriminator(perturbed_signals, np.ones((n_samples,)))))
    ...
    def predict_curr_discriminator_for_auth(self):
        signal=self.rf_system.tx_rx_real_symbol_block(self.rf_system.get_real_pretx_symbol_block(self.T//2), authorized=True, tx_id=1)
        return self.predict_discriminator(np.expand_dims(signal, 0)).squeeze()
    
    def predict_curr_discriminator(self):
        perturbed_signal=self.rf_system.tx_rx_real_symbol_block(self.agent.generator.sampling_sequence_alt(self.T, self.rf_system.get_real_pretx_symbol_block(self.T//2)), impersonator=True)
