...
pickle_path = 'data/pickles/t_10_snr_only_eps_0_03.pkl'
...
for n_authorized in [10]:
    ...
    for snr in [0,5,10,20,50]:
        ...
        res.append({"cnvg_val": trainer.test_curr_discriminator_batch(), "steps": 0})
        ...
        for epsilon in eps:
            for beta in betas:
                for reward_type in reward_types:
                    print("t: "+str(n_authorized)+", snr: "+str(snr)+", eps: "+str(epsilon))
                    if thr:
                        res.append({"cnvg_val": trainer.test_curr_discriminator_batch(), "steps": 0})
                        continue
                    ...
                    res.append({"cnvg_val": cnvg_val, "steps": steps})
                    if(cnvg_val < 0.00001):
                        thr=True
        res_t[snr]=res
