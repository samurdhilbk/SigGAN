...
n_symbols = 256 // 2
sess_name = 'test_realistic_t_10'
pickle_path = 'data/pickles/realistic_t_10_eps_0_08_beta_15000_reward_false_snr.pkl'
dump_pickle_each_loop = True
sensitivity = 0.02
data_pickle = {}
#data_pickle["eps"]=[0.2, 0.12, 0.10, 0.08, 0.07, 0.06, 0.05, 0.01]
data_pickle["snr"]=[0, 5, 10, 20, 50]
...
for n_authorized in [10]:
    ...
    for snr in [50, 20, 10, 5, 0]:
        ...
        sig_rd_train,sig_rd_valid,sig_rd_test=split(fft(sig_rd),n_train,n_valid,n_test)
        ...
        sig_impersonate_ad_fft = fft(sig_impersonate_ad)
        ...
        im=trainer.test_discriminator(sig_impersonate_ad_fft, np.zeros((sig_impersonate_ad.shape[0],)))

        au_act=np.mean(trainer.predict_discriminator(fft(sig_auth)))
        im_act=np.mean(trainer.predict_discriminator(sig_impersonate_ad_fft))
