...
class RFSystem:
    ...
    def __init__(self, n_authorized = 1, n_unauthorized = 1, mod = 'qpsk', snr = 20,
        ...
        if sess_dir != "":
            self.auth_transmitters, self.unauth_transmitters, self.impersonator_transmitter = self.load_transmitters_from_file('./sessions/' + sess_dir)
            ...
            self.store_transmitters(self.auth_transmitters+self.unauth_transmitters+[self.impersonator_transmitter], self.full_sess_dir)
            store_configuration(self.full_sess_dir+'/config.txt', mod = mod, snr = snr, coef_set = coef_set)
...
#         self.impersonator_transmitter = self.randomly_generate_transmitters(1, False)[0]
    ...
    def get_received_symbol_block(self, n_symbols, tx_id, authorized, dynamic_channel=True):
    ...
    def transmit_symbol_block(self, symbols, tx_id = 0, authorized = True, impersonator = False, dynamic_channel=True):
