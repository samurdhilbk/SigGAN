...
class Environment(object):
    ...
    def process_reward(self, perturbed_signal):
        ...
        if self.binary_reward: reward =  1 if ((self.discriminator.predict(self.fft(trans_signal).reshape(-1,self.T)).squeeze()) > 0.5) else 0
        else: reward = ((self.discriminator.predict(self.fft(trans_signal).reshape(-1,self.T)).squeeze()) - 0.0) 
        return reward
