...
class Generator():
    def __init__(self, sess, H, lr=1e-3):
        self.sess = sess
        self.H = H
        self.lr = lr
        self.build_graph()
    ...
    def build_graph(self):
        ...
        #self.sample = tf.clip_by_value(self.normal_dist.sample(), clip_value_min=state_in_-self.epsilon*tf.abs(state_in_), clip_value_max=state_in_+self.epsilon*tf.abs(state_in_))
        
        log_prob = tf.reshape(self.normal_dist.log_prob(action), (-1,1)) # (B, )
        loss = - tf.reduce_sum(log_prob * tf.cumsum(reward, axis=-1))
        loss -= 10000 * self.normal_dist.entropy()
        
        global_step = tf.Variable(0, trainable=False)
        lr_schedule = tf.train.exponential_decay(self.lr,decay_steps=20,global_step=global_step,decay_rate=0.95)
...
def Discriminator(dropout=0.5):
    ...
    x = resnet(x,16,(3,2),'1',keras.regularizers.l2(0.001))
    # # x = resnet(x,32,(3,2),'2')
    x = MaxPool2D((2,1))(x)
    i=0


    x = resnet(x,32,(3,2),'3_{}'.format(i),keras.regularizers.l2(0.002))
    #     x = resnet(x,32,(3,2), '4_{}'.format(i),keras.regularizers.l2(0.001))
    x = MaxPool2D((2,1))(x)
    ...
    x = Dense(20, activation='relu',kernel_regularizer = keras.regularizers.l2(0.002))(x)
    x = Dropout(0.5)(x)
